...
class LoadBalancerTest {
    ...
    @Test
    fun unhealthyServerIgnored() = withDefer {
        // Given
    ...
    @Test
    fun complexUrlForwardingWorks() = withDefer {
        // Given
        val loadBalancer = LoadBalancerCli()
        defer { loadBalancer.stop() }
        loadBalancer.main(
        ...
        withSingleEndpointServer(
            port = 9000,
            endpoint = "/api/endpoint",
            returnResponse = "Test from server 1",
            expectRequest = { exchange ->
        ...
        // When
        val response =
            URL("http://localhost:8000/endpoint?query=parm&value%20+#fragment").openConnection() as HttpURLConnection
    ...
    @Test
    fun badGatewayErrorWhenNoHealthyDownstreamIsAvailable() = withDefer {
        // Given
        val loadBalancer = LoadBalancerCli()
        defer { loadBalancer.stop() }
        loadBalancer.main(
    ...
    @Test
    fun testUnhealthyServerCanRecover() = withDefer {
        // Given
        val loadBalancer = LoadBalancerCli()
        defer { loadBalancer.stop() }
        loadBalancer.main(
